...
exp_name = 'lambda_500_rr_balanced_NAF'
feature_subset = 'rr'
...
print(f'{exp_name}: Starting post train analysis')
...
haifa_model = HSICClassifier(num_classes=2, feature_len=train_loader.dataset.feature_len,
                             feature_opt=feature_opt, gap_norm_opt='batch_norm').to(device)

haifa_model.load_state_dict(torch.load(os.path.join(file_dir, f'{exp_name}_params.pkl'), map_location='cpu'))
...
gap_train, gap_val, gap_test = generate_gap(train_loader, val_loader, test_loader, model=haifa_model,
                                            file_dir=file_dir, file_name=exp_name, device=device)
...
if run_rep2label:
    ...
    with open(os.path.join(file_dir, f'{exp_name}_ugap_test.pkl'), 'rb') as handle:
        test_gaps = pkl.load(handle)

    with open(os.path.join(file_dir, f'{exp_name}_ugap_train.pkl'), 'rb') as handle:
        train_gaps = pkl.load(handle)

    with open(os.path.join(file_dir, f'{exp_name}_ugap_val.pkl'), 'rb') as handle:
...
print(f'{exp_name}: Finished post train analysis')
